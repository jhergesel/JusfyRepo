...
export const reducer = (state = INITIAL_STATE, action) => {
    switch (action.type) {
        case "LOAD_CALCULATION_FAILED":
            return {
                ...state,
                calculation: INITIAL_STATE.calculation,
                calculation_loading: false,
            };
...
function* loadCalculation({
    ...
    try {
        const response = yield axios.post(
            process.env.REACT_APP_API_URL + `/calculations/${payload.id}`,
            payload
        );
        if (!response.data.success) {
            yield put({
                type: "LOAD_CALCULATION_FAILED"
            });
            return;
        }
        yield put({
