...
class CurrencyInput extends Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        ...
        // Se o valor não mudou, evita recalcular a posição do cursor
        if (prevState.maskedValue === this.state.maskedValue) {
            return;
        }
        ...
        let selectionStart = Math.max(
            minPos,
            Math.min(this.inputSelectionStart, selectionEnd)
        );
        ...
        // Só atualiza a seleção se for necessário
        if (
    ...
    handleCopy = event => {
        ...
        if (!selection) {
            event.preventDefault(); // Impede a cópia do valor padrão
            event.clipboardData.setData("text/plain", this.state.maskedValue); // Define o valor correto
        }
    };
