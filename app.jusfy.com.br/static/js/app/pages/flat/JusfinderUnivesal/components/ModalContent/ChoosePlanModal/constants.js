...
function useSubscriptionPlans(hasJusProcessos = false, processCount = 200) {
    ...
    const cycle = billingCycle === BILLING_CYCLES.ANNUAL ? "annual" : "monthly";
    ...
    const finalPlanPrices = isFeatureFlagEnable ?
        (() => {
            const fallbackCycle = billingCycle === BILLING_CYCLES.ANNUAL ? "annually" : "monthly";
            return {
                ultimate: choosePlanModalAdapter(ultimatePlan) || BASE_PRICING["ultimate"][fallbackCycle],
            };
        })() :
        planPrices;
