...
import {
    MAX_CREDITS_AMOUNT_VALUE,
    MIN_CREDITS_AMOUNT_VALUE,
    PRICE_CREDITS,
} from "./constants";
...
const BuyCreditsModal = ({
    ...
    const [creditsAmount, setCreditsAmount] = useState(
        creditQuantity || MIN_CREDITS_AMOUNT_VALUE
    );
    ...
    const queryType = () => {
        ...
        return (
            queryOptions.get(queryOption ? .identification) ||
    ...
    const onSubmit = card => {
        ...
        axios
            .post(`${process.env.REACT_APP_API_URL}/queries/purchase_credits`, {
                card_id: card.hash,
                product_identification: "universal_credits",
            ...
            .then(response => {
                ...
                HandleEvent("Universal Query Credit Bought", {
                    payment_option,
    ...
    const pricePreview = PRICE_CREDITS * creditsAmount;

    useEffect(() => {
        if (isLoading && queryOption) {
    ...
    useEffect(() => {
        HandleEvent("Universal Buy Query Credit Modal Opened", {
    ...
    const openPlanModal = () => {
        ...
        closeModal();
        HandleEvent("Universal About Information Plans Clicked");
    };
    ...
    return ( <
        ...
        p > Créditos < /p> <
        h1 > {
            isNewCard ? "Novo cartão" : "Comprar créditos"
        } < /h1> <
        /S.Header>
...

        {
            isNewCard ? ( <
                ...
                SelectCardForm buttonText = "Realizar compra"
                onNewCard = {
                    () => setIsNewCard(true)
                }
                onSubmit = {
        ...
        <
        ContentCardInfos > {
            PLAN_OPTIONS.plan.map((plan, index) => ( <
                CardPlan key = {
                    `${plan.name}-${index}`
                } >
                <
