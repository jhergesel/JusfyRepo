...
export default function ChoosePlan(props) {
    ...
    const isOffer = subscription ? .subscription_status ? .isOffer;
    ...
    const chooseProduct = plan => {
        ...
        const invalidIdentifiers = subscriptionPlan && (isNil(subscriptionPlan.id) || isNil(subscriptionPlan.gateway_id));
        const hasNonNumericId = subscriptionPlan && typeof subscriptionPlan.id !== 'number';
        const hasErrors = subscriptionPlanNotFound || invalidIdentifiers || hasNonNumericId;

        if (hasErrors) {
            toast.error("Plano não encontrado. Por favor, tente novamente ou contate o suporte.");
