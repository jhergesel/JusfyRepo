...
export const UseQueryHistory = () => {
    ...
    const reprocessQuery = useCallback(
        async (queryReprocess) => {
            ...
            try {
                ...
                HandleEvent('Repeat Query Clicked', {
    ...
    const deleteQuery = useCallback(
        async (data) => {
            try {
                ...
                HandleEvent('Query Performed Deleted', {
    ...
    const openInNewTab = useCallback(
        (data) => {
            HandleEvent('Query Report Downloaded', {
                query_type: data ? .feature_type_name,
                where: 'history',
            });
    ...
    const featureTypesUrl = useCallback(
        (row, context = '') => {
            if (row.status === 'pending' || row.status === 'error' || !row.status) {
                return;
            }

            HandleEvent('Query Performed Opened', {
                where: context,
    ...
    const reloadPage = useCallback(() => {
        setShouldReload(true);
        HandleEvent('Query Refresh Button Clicked');
